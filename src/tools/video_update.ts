...
server.tool(
    ...
    async ({ access_key, pkg }, { extra }) => {
        const env = extra as any;
...
server.tool(
    ...
    async ({ access_key, ...params }, { extra }) => {
        const env = extra as any;
        const client = new WecandeoClient(env.WECANDEO_ACCESS_KEY);
        const result = await client.get("https://api.wecandeo.com/info/v1/video/set/detail.json", {
            key: env.WECANDEO_ACCESS_KEY,
            access_key,
            ...params,
        });
...
server.tool(
    ...
    async ({ access_key, enable, ...params }, { extra }) => {
        const env = extra as any;
        const client = new WecandeoClient(env.WECANDEO_ACCESS_KEY);
