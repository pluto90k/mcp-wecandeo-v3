...
Claude Desktop, Claude Code, Cursor 등 MCP 클라이언트에서 Wecandeo 비디오 관리 도구 37개를 Stdio 모드로 사용할 수 있습니다.

- **홈페이지**: [https://www.wecandeo.com](https://www.wecandeo.com)
- **API 가이드**: [https://support.wecandeo.com](https://support.wecandeo.com)

> [!WARNING]
> **V3는 더 이상 업데이트되지 않습니다.** 본 패키지는 기존 사용자를 위해 유지되는 버전입니다.
> 신규 도입 및 새로운 기능은 **V4(`@pluto90/wecandeo-v4-mcp`)** 를 사용해 주세요.
> 통계/AI 기능은 각각 `@pluto90/wecandeo-v4-analytics-mcp`, `@pluto90/wecandeo-v4-ai-mcp` 로 제공됩니다.
...
| 변수 | 설명 |
|---|---|
| `WECANDEO_API_KEY` | Wecandeo API 키 (필수, `WECANDEO_ACCESS_KEY` 와 동일하게 지원) |

- **API 액세스 키 확인 방법**: [비디오팩 API 시작하기](https://support.wecandeo.com/reference/videopack-api-getting-started) 가이드를 참고하세요.
...
```bash
WECANDEO_API_KEY=your_key npx @pluto90/wecandeo-v3-mcp
```
...
| | `wecandeo_encoding_cancel` | 인코딩 개별 취소 [NEW] |
| | `wecandeo_encoding_cancel_all` | 인코딩 일괄 취소 [NEW] |
| | `wecandeo_encoding_retry` | 인코딩 개별 재시도 [NEW] |
| | `wecandeo_encoding_retry_all` | 인코딩 일괄 재시도 [NEW] |
| | `wecandeo_encoding_list` | 전체 인코딩 요청 조회 [NEW] |
